...
Begin VB.Form frmBoletimAdm2 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Boletim Administrativo - Final"
   ClientHeight    =   8160
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7380
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cbo_sala 
         Height          =   315
         ItemData        =   "frmBoletimAdm2.frx":0000
         Left            =   555
         List            =   "frmBoletimAdm2.frx":0002
         TabIndex        =   9
         Top             =   630
         Width           =   4545
      End
      Begin VB.CommandButton cmdProcura 
         Height          =   315
         Left            =   5100
         Picture         =   "frmBoletimAdm2.frx":0004
         Style           =   1  'Graphical
         TabIndex        =   8
         TabStop         =   0   'False
   ...
   Begin VSFlex7LCtl.VSFlexGrid vsfBoletim 
      ...
      FormatString    =   $"frmBoletimAdm2.frx":038E
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmBoletimAdm2"
...
Private Sub cmdProcura_Click()
    Dim clsBoletim2 As New clsBoletim2
    Dim oRsAux      As ADODB.Recordset
    Dim dtDif       As Integer
    ...
    Set clsBoletim2.ConexaoADO = dbConnect
    clsBoletim2.data = dtpData.Value
    ...
    dtDif = DateDiff("d", dtpData.Value, Date)
    
    If clsBoletim2.VerificaBol And dtDif <= 2 And dtDif >= 0 Then
        If MsgBox("Boletim do dia " + Format(dtpData.Value, "dd/mm/yyyy") + _
                  " já existe. Gera novo movimento?", vbYesNo, App.ProductName) = vbYes Then
            clsBoletim2.GeraBol
        End If
    ElseIf Not clsBoletim2.VerificaBol Then
        clsBoletim2.GeraBol
    End If
    
    If clsBoletim2.FilmesBol(oRsAux) Then
        If Not (oRsAux.BOF And oRsAux.EOF) Then
            Do While Not oRsAux.EOF
                If oRsAux.Fields("bol_status").Value = "N" Then
                    cbo_sala.AddItem oRsAux.Fields("Sala - Filme").Value
                ElseIf oRsAux.Fields("bol_status").Value = "S" Then
                    cbo_sala.AddItem "* " & oRsAux.Fields("Sala - Filme").Value
                ElseIf oRsAux.Fields("bol_status").Value = "P" Then
                    cbo_sala.AddItem "# " & oRsAux.Fields("Sala - Filme").Value
                ElseIf oRsAux.Fields("bol_status").Value = "Q" Then
                    cbo_sala.AddItem "#* " & oRsAux.Fields("Sala - Filme").Value
                End If
                'cbo_sala.AddItem oRsAux.Fields("Sala - Filme").Value
...
Private Sub Form_Activate()
    ...
    If inicio Then
        ...
        dtpData.Value = gerais.DataRefAtu
        Call cmdProcura_Click
        inicio = False
    End If
End Sub
...
Private Sub cmdOK_Click()
    ...
    impriminto = True
    Call ImprimeBoletim(dtpData.Value, sala.Item(cbo_sala.ListIndex + 1), filme.Item(cbo_sala.ListIndex + 1))
    impriminto = False
End Sub

Private Sub ImprimeBoletim(ByVal dData As Date, ByVal iSala As Integer, ByVal iFilme As Long)

    On Error GoTo ImprimeBoletim_erro
    
    Dim clsBoletim  As New clsBoletim2
    ...
    Do While Not oRsCapa.EOF()
        ...
        vsfBoletim.AddItem (LinhaTitulo & Chr(9) & "BOLETIM" & Chr(9) & Chr$(10))
        ReDim Preserve linha(1 To UBound(linha) + 1) As String
        ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
        linha(UBound(linha)) = "ADMINISTRATIVO"
        tplinha(UBound(tplinha)) = "T"
        linha(UBound(linha)) = "BOLETIM"
        tplinha(UBound(tplinha)) = "T"
        
        vsfBoletim.AddItem (LinhaTitulo & Chr(9) & "ADMINISTRATIVO" & Chr(9) & Chr$(10))
        ReDim Preserve linha(1 To UBound(linha) + 1) As String
        ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
        linha(UBound(linha)) = "ADMINISTRATIVO"
        tplinha(UBound(tplinha)) = "T"
        ...
        If Not IsNull(oRsCapa.Fields("prg_dt_ini").Value) Then
            ...
            tplinha(UBound(tplinha)) = "N"
        Else
            vsfBoletim.AddItem (LinhaNormal & Chr(9) & Alinha("Periodo ..: Excluido", COLUNAS_IMP, esquerda) & Chr(9) & Chr$(10))
            ReDim Preserve linha(1 To UBound(linha) + 1) As String
            ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
            linha(UBound(linha)) = Alinha("Periodo ..:  Excluido", COLUNAS_IMP, esquerda)
            tplinha(UBound(tplinha)) = "N"
        End If
...
'                dValorIng = oRsAux.Fields("ing_valor")
'            End If
'
'            dValorTotal = dValorTotal + oRsAux.Fields("ing_valor") * oRsAux.Fields("qtde")
...
'                                Alinha(Format(oRsAux.Fields("ing_valor") * oRsAux.Fields("qtde"), "0.00"), 9, Direita, " ") & Chr(9) & Chr$(10))
'            ReDim Preserve linha(1 To UBound(linha) + 1) As String
'            ReDim Preserve tpLinha(1 To UBound(tpLinha) + 1) As String
...
'        oRsAux.Close
'
'        vsfBoletim.AddItem (String(COLUNAS_IMP, "_") & Chr(9) & "" & Chr(9) & Chr$(10))
'        ReDim Preserve linha(1 To UBound(linha) + 1) As String
'        ReDim Preserve tpLinha(1 To UBound(tpLinha) + 1) As String
...
'                                       Alinha(Format(dValorIng, "0.00"), 6, Direita, " ") & " " & _
'                                       Alinha(oRsAux.Fields("qtde"), 4, Direita, " ") & " " & _
'                                       Alinha(Format(oRsAux.Fields("ing_valor") * oRsAux.Fields("qtde"), "0.00"), 9, Direita, " ")
'                tpLinha(UBound(tpLinha)) = "N"
'
'                oRsAux.MoveNext
'
...
'******Vendas totais
        ...
        If Not clsBoletim.VendasTotal(oRsAux) Then
            MsgBox clsBoletim.MensagemErro, vbCritical, App.ProductName
            GoTo ImprimeBoletim_fim
        End If
        ...
        vsfBoletim.AddItem (LinhaDupla & Chr(9) & "TOTAL VENDAS" & Chr(9))
        ReDim Preserve linha(1 To UBound(linha) + 1) As String
        ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
        linha(UBound(linha)) = "TOTAL VENDAS"
        tplinha(UBound(tplinha)) = "S"
        ...
        vsfBoletim.AddItem (LinhaNormal & Chr(9) & Alinha("TOTAL VENDAS", 23, esquerda, " ") & " " & _
                            Alinha(iQtdeTotal, 4, Direita, " ") & " " & _
                            Alinha(Format(dValorTotal, "0.00"), 9, Direita, " ") & Chr(9) & Chr$(10))
        ReDim Preserve linha(1 To UBound(linha) + 1) As String
        ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
        linha(UBound(linha)) = Alinha("TOTAL VENDAS", 23, esquerda, " ") & " " & _
                               Alinha(iQtdeTotal, 4, Direita, " ") & " " & _
                               Alinha(Format(dValorTotal, "0.00"), 9, Direita, " ")
        tplinha(UBound(tplinha)) = "N"
        ...
        If Not oRsAux.EOF() Then
            ...
            Do While Not oRsAux.EOF()
                If sData <> Format(oRsAux.Fields("sre_data"), "dd/mm/yyyy") Then
                    ...
                    ReDim Preserve linha(1 To UBound(linha) + 1) As String
                    ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
                    linha(UBound(linha)) = Alinha("PARA DIA", 26, esquerda, ".") & " " & sData '& Space(8)
                    tplinha(UBound(tplinha)) = "N"
                End If
...
'            vsfBoletim.AddItem (Chr$(10))
            ...
            Do While Not oRsAux.EOF()
                ...
                ReDim Preserve linha(1 To UBound(linha) + 1) As String
                ReDim Preserve tplinha(1 To UBound(tplinha) + 1) As String
                linha(UBound(linha)) = Trim(oRsAux.Fields("cat_nm").Value & " - Iniciante : " & oRsAux.Fields("ctc_ini_cont").Value)
                tplinha(UBound(tplinha)) = "N"
            
'                vsfBoletim.AddItem (LinhaNormal & Chr(9) & Alinha("Iniciante : " & oRsAux.Fields("ctc_ini_cont").Value, COLUNAS_IMP, Esquerda) & Chr(9) & Chr$(10))
'                ReDim Preserve linha(1 To UBound(linha) + 1) As String
'                ReDim Preserve tpLinha(1 To UBound(tpLinha) + 1) As String
'                linha(UBound(linha)) = ""
'                tpLinha(UBound(tpLinha)) = "N"
