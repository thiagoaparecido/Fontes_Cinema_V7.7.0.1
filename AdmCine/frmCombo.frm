...
Private Sub cmdComandos_Click(ByVal iButtonClicked As Comandos.eButtonClicked, Cancel As Boolean)
    ...
    Select Case iButtonClicked
        ...
        Case ButtonExclui
            If VSFlexGrid.RowSel > 0 And VSFlexGrid.Rows > 1 Then
                If MsgBox("Confirma exclusão do Combo selecionado?", vbYesNo + vbQuestion + vbDefaultButton2, App.ProductName) = vbYes Then
                    If Exclui() Then
...
Private Sub CarregaControles()
    ...
    If VSFlexGrid.RowSel > 0 And VSFlexGrid.Rows > 1 Then
        ...
        txt_cbo_desc.Text = VSFlexGrid.TextMatrix(VSFlexGrid.RowSel, VSFlexGrid.ColIndex("Descrição"))
    End If
End Sub
...
Private Function Grava() As Boolean
    ...
    If sOperacao = "I" Then
        If Not clsTB_COMBO.Incluir() Then
            MsgBox "Não foi possível incluir o Combo!" & vbCrLf & clsTB_COMBO.MensagemErro, vbInformation, App.ProductName
            GoTo Grava_Fim
        End If
    Else
        clsTB_COMBO.cbo_cd = VSFlexGrid.TextMatrix(VSFlexGrid.RowSel, VSFlexGrid.ColIndex("cbo_cd"))
        If Not clsTB_COMBO.Alterar() Then
            MsgBox "Não foi possível alterar o Combo Selecionado!" & vbCrLf & clsTB_COMBO.MensagemErro, vbInformation, App.ProductName
            GoTo Grava_Fim
        End If
    End If
...
Grava_Erro:
    MsgBox "Erro de execução! 'Grava/frmCombos'", vbCritical, App.ProductName
...
Private Function Exclui() As Boolean
    ...
    If Not clsTB_COMBO.Excluir() Then
        MsgBox "Não foi possível excluir o Combo Selecionado!" & vbCrLf & clsTB_COMBO.MensagemErro, vbInformation, App.ProductName
        GoTo Exclui_Fim
    End If
...
Exclui_Erro:
    MsgBox "Erro de execução! 'Exclui/frmCombos'", vbCritical, App.ProductName
...
PreencheGrid_Erro:
    MsgBox "Erro de execução! 'PreencheGrid/frmCombos'", vbCritical, App.ProductName
...
Private Function Consiste() As Boolean
    ...
    If Trim(txt_cbo_desc.Text) = "" Then
        sMens = sMens & "Descrição do Combo deve ser informado!" & vbCrLf
    End If
