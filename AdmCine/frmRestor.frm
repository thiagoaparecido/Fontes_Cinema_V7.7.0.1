...
Private Sub cmdArq_Click()
    ...
    If Dir("C:\CINEMA", vbDirectory) <> Empty Then
        CommonDialog1.InitDir = "C:\CINEMA"
    ElseIf Dir("C:\BANCOTICKET\", vbDirectory) <> Empty Then
        CommonDialog1.InitDir = "C:\BANCOTICKET"
    Else
...
Private Sub cmdOK_Click()
        ...
        DoEvents
        'Pesquisando se existe o banco de dados para apaga-lo
    
        
        Dim wConect As String
                
        rs.Open "SELECT * FROM master.dbo.sysdatabases where name = 'CINEMA'", "FILE NAME=" & App.Path & "\Cinema.udl", adOpenDynamic, adLockReadOnly, adCmdText
        rs.Close
        On Error GoTo TrataErro
        
        wConect = Replace(rs.ActiveConnection, "Initial Catalog=cinema;", "Initial Catalog=master;")
        
        rs.Open "SELECT * FROM master.dbo.sysdatabases where name = 'CINEMA'", wConect, adOpenDynamic, adLockReadOnly, adCmdText
        
        If Not rs.EOF And Not rs.BOF Then  'Existe -> Apaga-lo
            
            Dim wSql As String
            wSql = "USE master "
            wSql = wSql + "ALTER DATABASE Cinema SET SINGLE_USER WITH ROLLBACK IMMEDIATE "
            
            If rs.State = 1 Then
                rs.Close
            End If
            rs.Open (wSql)
            wSql = wSql + "ALTER DATABASE Cinema SET SINGLE_USER "
            If rs.State = 1 Then
                rs.Close
            End If
            rs.Open (wSql)
            If rs.State = 1 Then
                rs.Close
            End If
            wSql = "USE MASTER DROP DATABASE Cinema"
            rs.Open (wSql)
        End If
            ...
            End If
            wSql = "USE master "
            wSql = wSql + "Restore Database Cinema "
            ...
            If rs.State = 1 Then
                rs.Close
            End If
            wSql = wSql + "ALTER DATABASE Cinema SET MULTI_USER "
            rs.Open (wSql)
...
TrataErro:
    ...
    If rs.State = 1 Then
        rs.Close
    End If
    rs.Open ("SELECT * FROM [master].[dbo].[sysdatabases] where name = 'Cinema'")
    
    If rs.RecordCount = 1 Then 'Existe -> Apaga-lo
        wSql = "USE master ALTER DATABASE Cinema SET MULTI_USER "
        rs.Open (wSql)
    End If
